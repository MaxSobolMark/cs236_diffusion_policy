        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                if action_buffer is None:
                    action_prediction_horizon = action.shape[1]
                    action_buffer = np.tile(
                        action[None], (action_prediction_horizon, 1, 1, 1)
                    )
                action_buffer = action_temporal_ensembling_add_action_to_buffer(
                ...
                env_action = action
                if action_temporal_ensemble:
                    env_action = action_temporal_ensembling_get_action_from_buffer(
                        action_buffer
                    )
                if self.abs_action:
...
def test_action_temporal_ensembling():
